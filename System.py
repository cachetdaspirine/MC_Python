...
libTriangle = cdll.LoadLibrary(
    str(pathlib.Path(__file__).parent.absolute()) + '/libTriangle.so')

libTriangle.CreateSystem.restype = POINTER(c_void_p)
libTriangle.CreateSystem.argtypes = [
    POINTER(c_int), c_int, c_int, c_double, c_double, c_double, c_double]
...
libTriangle.SetElasticConstant.argtypes = [
    c_double, c_double, c_double, c_double, POINTER(c_void_p)]
libTriangle.UpdateSystemEnergy.argtypes = [
...
libHexagon.CreateSystem.argtypes = [
    POINTER(c_int), c_int, c_int, c_double, c_double, c_double, c_double]
...
libHexagon.SetElasticConstant.argtypes = [
    c_double, c_double, c_double, c_double, POINTER(c_void_p)]
libHexagon.UpdateSystemEnergy.argtypes = [
    POINTER(c_void_p), POINTER(c_int), c_int, c_int]
...
class System:
    def __init__(self, State=None,
        ...
        if old_system == None:
            if not Parameter:
                ...
                self.None_Copy(State, Parameter.epsilon,
                               Parameter.k, Parameter.kc, Parameter.kA)
        else:
    ...
    def SetElasticConstant(self, Kmain=np.nan, Kcoupling=np.nan, epsilon=np.nan, KVOL=np.nan):
        ...
        if np.isnan(epsilon):
            ...
            epsilon1 = epsilon
        self.lib.SetElasticConstant(
            epsilon1, Kmain1, Kcoupling1, KVOL1, self.Adress)
    ...
    def PlotPerSpring(self, figuresize=(7, 5), Zoom=1., Colorbar=None):
        ...
        os.system('rm -rf ToPlot.txt')
        XC, YC = 0, 0
        X1, X2, Y1, Y2, C0, C1 = np.array([]), np.array([]), np.array(
